...
/*!
 *   @file       sofawrite.cpp
 *   @brief      Various code examples... Just adapt these pieces of code to
 * your needs...
...
/************************************************************************************/
/*!
 *  @brief          Example for testing whether a file match a SOFA convention
 * or not without raising any exception
 *
 */
/************************************************************************************/
static void TestFileConvention(const std::string &filename,
  ...
  const bool validSimpleFreeFieldHRIR =
      sofa::IsValidSimpleFreeFieldHRIRFile(filename);
  const bool validSimpleFreeFieldSOS =
      sofa::IsValidSimpleFreeFieldSOSFile(filename);
  const bool validSimpleHeadphoneIR =
      sofa::IsValidSimpleHeadphoneIRFile(filename);
  ...
  const bool validMultiSpeakerBRIR =
      sofa::IsValidMultiSpeakerBRIRFile(filename);
  const bool validSingleRoomDRIR = sofa::IsValidSingleRoomDRIRFile(filename);

  output << "netCDF               = " << sofa::String::bool2yesorno(validnetCDF)
         << std::endl;
  output << "SOFA                 = " << sofa::String::bool2yesorno(validSOFA)
         << std::endl;
  output << "SimpleFreeFieldHRIR  = "
         << sofa::String::bool2yesorno(validSimpleFreeFieldHRIR) << std::endl;
  output << "SimpleFreeFieldSOS   = "
         << sofa::String::bool2yesorno(validSimpleFreeFieldSOS) << std::endl;
  output << "SimpleHeadphoneIR    = "
         << sofa::String::bool2yesorno(validSimpleHeadphoneIR) << std::endl;
  output << "GeneralFIR           = "
         << sofa::String::bool2yesorno(validGeneralFIR) << std::endl;
  output << "GeneralFIRE          = "
         << sofa::String::bool2yesorno(validGeneralFIRE) << std::endl;
  output << "GeneralTF            = "
         << sofa::String::bool2yesorno(validGeneralTF) << std::endl;
  output << "MultiSpeakerBRIR     = "
         << sofa::String::bool2yesorno(validMultiSpeakerBRIR) << std::endl;
  output << "SingleRoomDRIR       = "
         << sofa::String::bool2yesorno(validSingleRoomDRIR) << std::endl;
}

/************************************************************************************/
/*!
 *  @brief          Example for displaying all informations about a NetCDFFile
 * file, in a fashion similar to matlab 'ncdisp' command
 *
 */
/************************************************************************************/
static void DisplayInformations(const std::string &filename,
  ...
  //==============================================================================
  {
    ...
    for (std::size_t i = 0; i < attributeNames.size(); i++) {
      ...
      output << tabSeparator << sofa::String::PadWith(attributeNames[i])
             << " = " << value << std::endl;
    }
  }
  ...
  //==============================================================================
  {
    ...
    for (std::size_t i = 0; i < variableNames.size(); i++) {
      ...
      const std::string dimsNames =
          file.GetVariableDimensionsNamesAsString(name);
      const std::string dims = file.GetVariableDimensionsAsString(name);

      output << tabSeparator << name << std::endl;
      output << tabSeparator << tabSeparator
             << sofa::String::PadWith("Datatype: ") << typeName << std::endl;
      output << tabSeparator << tabSeparator
             << sofa::String::PadWith("Dimensions: ") << dimsNames << std::endl;
      ;
      output << tabSeparator << tabSeparator << sofa::String::PadWith("Size: ")
             << dims << std::endl;
      ;
      ...
      if (attributeNames.size() > 0) {
        output << tabSeparator << tabSeparator
               << sofa::String::PadWith("Attributes: ") << dims << std::endl;
        ;
      }

      for (std::size_t j = 0; j < attributeNames.size(); j++) {
        output << tabSeparator << tabSeparator << tabSeparator;
        output << sofa::String::PadWith(attributeNames[j]) << " = "
               << attributeValues[j] << std::endl;
      }
    }
  }
}
...
/************************************************************************************/
/*!
 *  @brief          Example for creating a SOFA file with the
...
static void CreateSimpleFreeFieldHRIRFile() {
  ...
  for (unsigned int k = 0; k < sofa::Attributes::kNumAttributes; k++) {
    const sofa::Attributes::Type attType =
        static_cast<sofa::Attributes::Type>(k);
  ...
  /// add attribute specific to your convention (e.g. 'DatabaseName' for the
  /// 'SimpleFreeFieldHRIR' convention)
  {
...
int main(int argc, char *argv[]) {
  ...
  const std::string filename =
      "/Users/tcarpent/Desktop/BRIRcut/studio1_olivier_P00.sofa";
  // const std::string filename =
  // "/Users/tcarpent/Desktop/BRIR_AddAbsorbers_ArrayCentre_Emitters1to64.sofa";
